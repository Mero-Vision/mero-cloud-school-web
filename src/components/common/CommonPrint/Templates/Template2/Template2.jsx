...
const Header = ({ row }) => {
  ...
  return (
    <Box className="header">
      ...
      <Box>
        <Typography className="bold">{singleData?.name}</Typography>
  ...
  return (
    <Box className="details">
      ...
      <Box textAlign={"right"}>
        ...
        <Typography>Due Date : {changeDateFormat(row?.due_date)}</Typography>
      </Box>
    </Box>
  );
};
          ...
          {row?.products?.map((item, index) => (
            <TableRow key={item?.id}>
              ...
              <TableCell align="center">
                {findInArray(TAX_DATA, "value", item?.tax)?.label ?? "-"}
              </TableCell>
        ...
        <Box className={classes.totalBoxContainer}>
          <Box className={classes.totalItems}>
                ...
                return (
                  <Box key={item?.title} className={classes.singleItem}>
                    ...
                    <Typography>
                      {" "}
                      Rs{" "}
                      {returnNumberWithCommas(row?.extra?.[item?.value] ?? "-")}
                    </Typography>
                  </Box>
                );
              })}
            ...
            <Box className={[classes.singleItem, classes.grandTotal]}>
              ...
              <Typography>
                Rs {returnNumberWithCommas(row?.extra?.grandTotal)}
              </Typography>
            </Box>
          </Box>
        </Box>
      ...
      <Grid container spacing={4}>
        ...
        <Grid item xs={6}>
          <Box className="bankDetails">
            ...
            <Typography>Account Holder : Scodus Innovations</Typography>
            <Typography>Account Number : 02138912380122</Typography>
            <Typography>Address : Baluwatar, Kathmandu</Typography>
          </Box>
        </Grid>
      </Grid>
    </Box>
