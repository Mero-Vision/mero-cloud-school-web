...
const CommonInvoice = ({ id, SingleInvoice }) => {
  const uuid = useId();
  ...
  const handleNavigate = (res) => {
    navigate(
      `/sales/invoice/${replaceFunction(res?.customer_detail?.name)}?id=${
        res?.id
      }`
    );
  };
  ...
  const onSubmit = (data) => {
    const finalData = {
      ...
      id,
      invoice_date: changeDateFormat(data?.invoice_date, "YYYY-MM-DD"),
