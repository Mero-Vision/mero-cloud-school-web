...
export default memo(CommonAddTable);
...
function SingleRow({
  ...
  useEffect(() => {
    if (watch(`products.${index}.product_id`)) {
      ...
      setValue(
        `products.${index}.rate`,
        product?.sale_rate || watch(`products.${index}.rate`)
      );
      setValue(
        `products.${index}.account_head_id`,
        product?.sale_account ||
          watch(`products.${index}.account_head_id`) ||
          ACCOUNT_HEAD_DATA?.[0]?.value
      );
    }
  }, stringifyData([watch(`products.${index}.product_id`), watch(`products.${index}.account_head_id`)]));
  useEffect(() => {
    const amount =
  ...
  // useEffect(() => {
  //   setValue(`products.${index}.discount`, "");
  // }, [watch(`products.${index}.quantity`), watch(`products.${index}.rate`)]);
  ...
  return (
    <>
      {/* <CustomModal
        ...
        modalTitle={`New Product/ Services`}
